...
class ServiceInstanceRecentLogsTests {
	...
	void setUp() {
		...
		Envelope testEnvelope = Envelope.builder()
			...
			.log(Log.builder()
				.payload(new String(Base64.getEncoder().encode(this.expectedTestMessage.getBytes())))
				.type(LogType.OUT)
				.build())
			.build();
	...
	void shouldFetchLogs() {
		WebTestClient client = WebTestClient.bindToServer().baseUrl("http://localhost:" + this.port).build();

		client.get()
			.uri("/logs/{serviceInstanceId}/recentlogs", SERVICE_INSTANCE_ID)
			.exchange()
			.expectStatus()
			.isOk()
			.expectBody(String.class)
			.value(Matchers.containsString(this.expectedTestMessage));
	}
