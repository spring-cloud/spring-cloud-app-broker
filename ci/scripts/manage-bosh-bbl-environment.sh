...
readonly LASTPASS_ATTACH_ID=att-5798335231099672425-2
...
base64_encode_bbl_state(){
	...
	[[ -f "${CI_DIR}/.envrc-localci" ]] || (printf "Download .envrc-localci from LastPass using \"lpass show --notes ${LASTPASS_LOCALCI_CREDS_NOTE_ID} --attach=$(lpass show ${LASTPASS_LOCALCI_CREDS_NOTE_ID} | sed -n 3p | cut -d ':' -f1) --quiet\" in directory ${CI_DIR} before running script"; exit 1)
...
generate_bbl_state_dir_for_environment(){
	local -r bbl_env_suffix="${1}"
#	local -r local_env_file="${CI_DIR}/.envrc-localci"
	...
	if [[ -z "${CIRCLE_BUILD_NUM:-}" ]] ; then
		...
		while read line; do
			if [[ ${line} =~ ^export[[:space:]]BBL_${bbl_suffix_upper}.*  ]]; then
				eval "${line}"
			fi
		done < ${CI_DIR}/.envrc-localci
	fi
