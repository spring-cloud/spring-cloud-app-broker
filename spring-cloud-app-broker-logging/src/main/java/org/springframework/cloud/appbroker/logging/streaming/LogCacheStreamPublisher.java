...
/***
 * Class to Stream LocCache Envelopes using the same pattern as the <a href=
 * "https://github.com/cloudfoundry/log-cache-cli/blob/main/internal/command/tail.go#L134">loc-cache-cli</a>.
 *
 * @author Alberto C. Rios
 */
public class LogCacheStreamPublisher implements LogStreamPublisher<Envelope> {
	...
	public LogCacheStreamPublisher(CloudFoundryClient client, LogCacheClient logCacheClient,
	...
	protected Flux<Envelope> createApplicationStreamer(String applicationId) {
		return getApplicationName(applicationId).flatMapMany((appName) -> {
			long initialStartTime = Instant.now().minus(5, ChronoUnit.SECONDS).toEpochMilli() * 1_000_000L;
			return readLogCache(applicationId, initialStartTime).flatMapMany((initialResponse) -> {
	...
	private static AtomicLong getLastTimestamp(ReadResponse initialResponse, long initialStartTime) {
		return new AtomicLong(initialResponse.getEnvelopes()
			.getBatch()
			.stream()
	...
	private static void updateLastTimestampFromResponse(ReadResponse readResponse, AtomicLong lastTimestamp) {
		long maxTimestamp = readResponse.getEnvelopes()
			.getBatch()
			.stream()
			.mapToLong(org.cloudfoundry.logcache.v1.Envelope::getTimestamp)
			.max()
