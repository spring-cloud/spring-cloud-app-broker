...
public class CloudControllerStubFixture extends WiremockStubFixture {
	...
	private void stubGetPlatformInfo() {
		stubFor(get(urlEqualTo("/")).withMetadata(optionalStubMapping()).willReturn(ok().withBody(cc("get-root"))));

		stubFor(get(urlEqualTo("/v2/info")).withMetadata(optionalStubMapping())
			.willReturn(ok().withBody(cc("get-info"))));
	}

	public void stubFindTestOrg() {
		stubFor(get(urlPathEqualTo("/v2/organizations")).withMetadata(optionalStubMapping())
			.willReturn(ok().withBody(cc("list-organizations", replace("@org-guid", TEST_ORG_GUID)))));

		stubFor(get(urlPathEqualTo("/v2/organizations/" + TEST_ORG_GUID)).withMetadata(optionalStubMapping())
			.willReturn(ok().withBody(cc("get-organization", replace("@org-guid", TEST_ORG_GUID)))));
	}

	private void stubFindTestOrgV3() {
		stubFor(get(urlPathEqualTo("/v3/organizations")).withMetadata(optionalStubMapping())
			.willReturn(ok().withBody(cc("list-organizations-v3", replace("@org-guid", TEST_ORG_GUID)))));

		stubFor(get(urlPathEqualTo("/v3/organizations/" + TEST_ORG_GUID)).withMetadata(optionalStubMapping())
			.willReturn(ok().withBody(cc("get-organization-v3", replace("@org-guid", TEST_ORG_GUID)))));
	}

	public void stubFindSpace(String spaceName, String spaceGuid) {
		stubFor(get(urlPathEqualTo("/v2/spaces")).withQueryParam("q", equalTo("name:" + spaceName))
			.withMetadata(optionalStubMapping())
			.willReturn(ok().withBody(cc("list-spaces", replace("@org-guid", TEST_ORG_GUID),
					replace("@name", spaceName), replace("@space-guid", spaceGuid)))));

		stubFor(get(urlPathEqualTo("/v2/spaces/" + spaceGuid)).withMetadata(optionalStubMapping())
			.willReturn(ok().withBody(cc("get-space", replace("@name", "test"), replace("@org-guid", TEST_ORG_GUID),
					replace("@space-guid", spaceGuid)))));

		stubFor(get(urlPathEqualTo("/v2/spaces/" + spaceGuid + "/apps")).withQueryParam("page", equalTo("1"))
			.withMetadata(optionalStubMapping())
			.willReturn(ok().withBody(cc("empty-query-results"))));

		stubFor(get(urlPathEqualTo("/v2/spaces/" + spaceGuid + "/security_groups")).withMetadata(optionalStubMapping())
			.willReturn(ok().withBody(cc("get-space-security_groups"))));
	}
	...
	public void stubFindSpaceV3(String spaceName, String spaceGuid) {
		stubFor(get(urlPathEqualTo("/v3/spaces")).withQueryParam("names", equalTo(spaceName))
			.withQueryParam("organization_guids", equalTo(TEST_ORG_GUID))
			.withQueryParam("page", equalTo("1"))
			.willReturn(ok().withBody(cc("list-spaces-v3", replace("@org-guid", TEST_ORG_GUID),
					replace("@name", spaceName), replace("@space-guid", spaceGuid)))));

		stubFor(get(urlPathEqualTo("/v3/spaces/" + spaceGuid)).withMetadata(optionalStubMapping())
			.willReturn(ok().withBody(cc("get-space-v3", replace("@name", "test"), replace("@org-guid", TEST_ORG_GUID),
					replace("@space-guid", spaceGuid)))));

		stubFor(get(urlPathEqualTo("/v3/spaces/" + spaceGuid + "/apps")).withQueryParam("page", equalTo("1"))
			.withMetadata(optionalStubMapping())
			.willReturn(ok().withBody(cc("empty-query-results"))));

		stubFor(get(urlPathEqualTo("/v3/spaces/" + spaceGuid + "/security_groups")).withMetadata(optionalStubMapping())
			.willReturn(ok().withBody(cc("get-space-security_groups"))));
	}

	private void stubFindDomains() {
		stubFor(get(urlPathEqualTo("/v2/shared_domains")).withMetadata(optionalStubMapping())
			.willReturn(ok().withBody(cc("list-shared-domains", replace("@org-guid", TEST_ORG_GUID)))));
		...
		stubFor(get(urlPathEqualTo("/v2/organizations/" + TEST_ORG_GUID + "/spaces"))
			...
			.willReturn(ok().withBody(
					cc("list-spaces", replace("@org-guid", TEST_ORG_GUID), replace("@space-guid", TEST_SPACE_GUID)))));

		stubFor(get(urlPathEqualTo("/v2/organizations/" + TEST_ORG_GUID + "/space_quota_definitions"))
			.withQueryParam("page", equalTo("1"))
			.withMetadata(optionalStubMapping())
			.willReturn(ok().withBody(cc("list-organizations-quota", replace("@org-guid", TEST_ORG_GUID)))));

		stubFor(get(urlPathEqualTo("/v2/quota_definitions/" + TEST_QUOTA_DEFINITION_GUID))
			.withMetadata(optionalStubMapping())
			.willReturn(ok().withBody(cc("get-organizations-quota", replace("@org-guid", TEST_ORG_GUID)))));
	}
	...
	public void stubCreateSpace(final String spaceName, final String spaceGuid) {
		stubFor(post(urlPathEqualTo("/v2/spaces"))
			...
			.willReturn(ok().withBody(cc("get-space", replace("@name", spaceName), replace("@space-guid", spaceGuid),
	...
	public void stubAppExistsInSpace(final String appName, final String spaceGuid) {
		stubFor(get(urlPathEqualTo("/v2/apps/" + appGuid(appName))).withMetadata(optionalStubMapping())
			.willReturn(ok().withBody(cc("get-app-STAGED", replace("@name", appName)))));

		stubFor(get(urlPathEqualTo("/v2/spaces/" + spaceGuid + "/apps")).withQueryParam("q", equalTo("name:" + appName))
			.withQueryParam("page", equalTo("1"))
			.willReturn(
					ok().withBody(cc("list-space-apps", replace("@name", appName), replace("@guid", appGuid(appName)),
							replace("@space-guid", spaceGuid), replace("@stack-guid", stackGuid(appName))))));

		stubFor(get(urlPathEqualTo("/v2/apps/" + appGuid(appName) + "/instances"))
			.willReturn(ok().withBody(cc("get-app-instances"))));

		stubFor(get(urlPathEqualTo("/v2/apps/" + appGuid(appName) + "/summary")).withMetadata(optionalStubMapping())
			.willReturn(
					ok().withBody(cc("get-app-summary", replace("@name", appName), replace("@guid", appGuid(appName)),
							replace("@stack-guid", stackGuid(appName)), replace("@route-guid", routeGuid(appName))))));

		stubFor(get(urlPathEqualTo("/v2/apps/" + appGuid(appName) + "/stats"))
			.willReturn(ok().withBody(cc("get-app-stats", replace("@name", appName)))));

		stubFor(get(urlPathEqualTo("/v2/stacks/" + stackGuid(appName)))
			.willReturn(ok().withBody(cc("get-stack", replace("@guid", stackGuid(appName))))));
	}

	public void stubAppExistsInSpaceV3(final String appName, final String spaceGuid) {
		stubFor(get(urlPathEqualTo("/v3/apps/" + appGuid(appName))).withMetadata(optionalStubMapping())
			.willReturn(ok().withBody(cc("get-app-STAGED-v3", replace("@name", appName)))));
	...
	public void stubAppExistsWithBackingService(final String appName, final String serviceInstanceName,
		...
		stubFor(get(urlPathEqualTo("/v2/apps/" + appGuid(appName) + "/summary"))
			.willReturn(ok().withBody(cc("get-app-summary-with-backing-service", replace("@name", appName),
					replace("@service-instance-name", serviceInstanceName),
					replace("@service-guid", serviceGuid(serviceName)), replace("@plan-guid", planGuid(planName)),
					replace("@guid", appGuid(appName)), replace("@stack-guid", stackGuid(appName)),
	...
	private void stubGetV3App(String appName) {
		stubFor(get(urlPathEqualTo("/v3/apps/" + appGuid(appName))).willReturn(ok()
	...
	private void stubCreatePackage(String appName) {
		stubFor(post(urlPathEqualTo("/v3/packages"))
			.withRequestBody(matchingJsonPath("$.[?(@.relationships.app.data.guid == '" + appGuid(appName) + "')]"))
			.willReturn(ok().withBody(cc("get-package-READY", replace("@guid", packageGuid(appName)),
	...
	private void stubGetPackage(String appName) {
		stubFor(get(urlPathEqualTo("/v3/apps/" + appGuid(appName) + "/packages"))
			...
			.willReturn(ok().withBody(cc("get-packages-READY", replace("@package-guid", packageGuid(appName))))));
	}

	private void stubCreateBuild(String appName) {
		stubFor(post(urlPathEqualTo("/v3/builds"))
			.withRequestBody(matchingJsonPath("$.[?(@.package.guid == '" + packageGuid(appName) + "')]"))
			.willReturn(ok().withBody(cc("get-build-STAGED", replace("@guid", buildGuid(appName)),
					replace("@package-guid", packageGuid(appName)), replace("@app-guid", appGuid(appName))))));
	...
	private void stubCreateDeployment(String appName) {
		stubFor(post(urlPathEqualTo("/v3/deployments"))
			.withRequestBody(matchingJsonPath("$.[?(@.relationships.app.data.guid == '" + appGuid(appName) + "')]"))
			.willReturn(ok().withBody(cc("get-deployment-DEPLOYED", replace("@guid", deploymentGuid(appName)),
					replace("@app-guid", appGuid(appName))))));
	...
	private void stubCreateAppMetadata(String appName, ContentPattern<?>... appMetadataPatterns) {
		...
		stubFor(mappingBuilder.willReturn(
				ok().withBody(cc("get-app-STOPPED", replace("@name", appName), replace("@guid", appGuid(appName))))));
	}

	private void stubMapRouteToApp(String appName, String host) {
		stubFor(get(urlPathEqualTo("/v2/apps/" + appGuid(appName) + "/routes")).withQueryParam("page", equalTo("1"))
			.willReturn(ok().withBody(cc("empty-query-results"))));

		stubFor(get(urlPathEqualTo("/v2/routes")).withQueryParam("q", equalTo("domain_guid:" + TEST_ORG_GUID))
			.withQueryParam("q", equalTo("host:" + host))
			.withQueryParam("page", equalTo("1"))
			.willReturn(ok().withBody(cc("empty-query-results"))));

		stubFor(post(urlPathEqualTo("/v2/routes")).withRequestBody(matchingJsonPath("$.[?(@.host == '" + host + "')]"))
			.willReturn(created()
	...
	private void stubUploadAppBits(String appName) {
		stubFor(put(urlPathEqualTo("/v2/resource_match")).withMetadata(optionalStubMapping())
			.willReturn(ok().withBody("[]")));

		stubFor(put(urlPathEqualTo("/v2/apps/" + appGuid(appName) + "/bits")).withQueryParam("async", equalTo("true"))
			.willReturn(created().withBody(cc("put-app-bits", replace("@guid", appName + "-JOB-GUID")))));
	}
	...
	private void stubCreateRouteInSpace(final String appName, final String spaceGuid) {
		stubFor(get(urlPathEqualTo("/v2/private_domains")).withQueryParam("page", equalTo("1"))
			.withMetadata(optionalStubMapping())
			.willReturn(ok().withBody(cc("empty-query-results"))));

		stubFor(get(urlPathEqualTo("/v2/spaces/" + spaceGuid + "/services")).withQueryParam("page", equalTo("1"))
			.willReturn(ok().withBody(cc("list-space-services").replace("@service-guid", "SERVICE-ID")
				.replace("@service-name", "db-service"))));

		stubFor(post(urlPathEqualTo("/v2/routes"))
			.willReturn(ok().withBody(cc("post-route").replace("@guid", routeGuid(appName)))));

		stubFor(put(urlPathEqualTo("/v2/apps/" + appGuid(appName) + "/routes/" + routeGuid(appName)))
			.willReturn(ok().withBody(cc("get-app-summary"))));
	}
	...
	public void stubDeleteApp(String appName) {
		stubFor(delete(urlPathEqualTo("/v2/routes/" + appName + "-ROUTE-GUID"))
			.willReturn(ok().withBody(cc("get-route", replace("@guid", routeGuid(appName))))));

		stubFor(delete(urlPathEqualTo("/v2/apps/" + appGuid(appName))).willReturn(noContent()));
	}
	...
	private void stubServiceInstanceExists(String serviceInstanceId, String serviceInstanceName, String serviceName,
		...
		stubFor(get(urlPathEqualTo("/v2/spaces/" + spaceGuid + "/service_instances"))
			...
			.willReturn(ok().withBody(cc("list-space-service_instances", replace("@space-guid", spaceGuid),
					replace("@service-guid", serviceGuid(serviceName)), replace("@plan-guid", planGuid(planName)),
					replace("@name", serviceInstanceName), replace("@guid", serviceInstanceId)))));
	}
	...
	public void stubGetBackingServiceInstanceFromSpace(String serviceInstanceName, String serviceName, String planName,
			String spaceGuid) {
	...
	public void stubServiceExistsInSpace(String serviceName, String planName, String spaceGuid) {
		stubFor(get(urlPathEqualTo("/v2/spaces/" + spaceGuid + "/services"))
			...
			.withQueryParam("page", equalTo("1"))
			.willReturn(ok().withBody(cc("list-space-services").replace("@service-guid", serviceGuid(serviceName))
		...
		stubFor(get(urlPathEqualTo("/v2/service_plans"))
			...
			.withQueryParam("page", equalTo("1"))
			.willReturn(ok().withBody(cc("list-service-plans").replace("@service-guid", serviceGuid(serviceName))
	...
	public void stubCreateServiceInstance(String serviceInstanceName) {
		stubFor(post(urlPathEqualTo("/v2/service_instances")).withQueryParam("accepts_incomplete", equalTo("true"))
	...
	public void stubDeleteServiceInstance(String serviceInstanceName) {
		...
		stubFor(delete(urlPathEqualTo("/v2/service_instances/" + serviceInstanceGuid)).willReturn(ok()));
	}
	...
	public void stubCreateServiceBinding(String appName, String serviceInstanceName) {
		...
		stubFor(post(urlPathEqualTo("/v2/service_bindings"))
			...
			.willReturn(created().withBody(cc("get-service_binding", replace("@guid", serviceBindingGuid),
					replace("@app-guid", appGuid(appName)), replace("@service-instance-guid", serviceInstanceGuid)))));
	}
	...
	public void stubServiceBindingExists(String appName, String serviceInstanceName) {
		...
		stubFor(get(urlPathEqualTo("/v2/service_bindings"))
			...
			.withQueryParam("page", equalTo("1"))
			.willReturn(ok().withBody(cc("list-service-bindings", replace("@guid", serviceBindingGuid),
					replace("@app-guid", appGuid(appName)), replace("@service-instance-guid", serviceInstanceGuid)))));

		stubFor(get(urlPathEqualTo("/v2/apps/" + appGuid(appName) + "/service_bindings"))
			.withQueryParam("q", equalTo("service_instance_guid:" + serviceInstanceGuid))
			.withQueryParam("page", equalTo("1"))
			.willReturn(ok().withBody(cc("get-service_bindings", replace("@service-binding-guid", serviceBindingGuid),
					replace("@app-guid", appGuid(appName)), replace("@service-instance-guid", serviceInstanceGuid)))));
	}
	...
	public void stubAssociatePermissions() {
		stubFor(get(urlPathEqualTo("/v2/config/feature_flags/set_roles_by_username"))
			.willReturn(ok().withBody(cc("get-feature-flag-roles"))));

		stubFor(put(urlPathEqualTo("/v2/organizations/" + TEST_ORG_GUID + "/users")).willReturn(ok()));
	}
	...
	private void stubGetServiceAndGetPlan(String serviceName, String planName) {
		stubFor(get(urlPathEqualTo("/v2/service_plans/" + planGuid(planName)))
			.willReturn(ok().withBody(cc("get-service-plan").replace("@service-guid", serviceGuid(serviceName))
				.replace("@plan-name", planName)
				.replace("@plan-guid", planGuid(planName)))));

		stubFor(get(urlPathEqualTo("/v2/services/" + serviceGuid(serviceName)))
			.willReturn(ok().withBody(cc("get-service").replace("@service-guid", serviceGuid(serviceName))
				.replace("@service-name", serviceName))));
	}
	...
	public void stubSpaceExists(final String spaceName, final String spaceGuid) {
		stubFor(get(urlPathEqualTo("/v2/organizations/" + TEST_ORG_GUID + "/spaces"))
			...
			.willReturn(ok().withBody(cc("list-spaces", replace("@org-guid", TEST_ORG_GUID),
					replace("@space-guid", spaceGuid), replace("@name", spaceName)))));
	}
