...
class AppDeploymentDeleteServiceInstanceWorkflowTests {
	...
	void setUp() {
		...
		this.backingServices2 = BackingServices.builder()
			.backingService(BackingService.builder()
				.name("my-service2")
				.plan("a-plan2")
		...
		BrokeredServices brokeredServices = BrokeredServices.builder()
			...
			.service(BrokeredService.builder()
				...
				.planName("plan3")
				.apps(this.backingApps)
				.services(this.backingServices2)
				.target(this.targetSpec)
				.build())
			.build();

		this.deleteServiceInstanceWorkflow = new AppDeploymentDeleteServiceInstanceWorkflow(brokeredServices,
				this.backingAppDeploymentService, this.backingAppManagementService,
				this.backingServicesProvisionService, this.backingSpaceManagementService, this.targetService);
	}
	...
	void deleteServiceInstanceWithDeployedAppsAndBoundServicesSucceeds() {
		...
		given(this.backingAppDeploymentService.undeploy(eq(this.backingApps)))
		...
		StepVerifier.create(this.deleteServiceInstanceWorkflow.delete(request, response)).verifyComplete();

		verify(this.backingServicesProvisionService, Mockito.times(1)).deleteServiceInstance(any());
		verify(this.backingSpaceManagementService).deleteTargetSpaces(eq(Collections.emptyList()));
	...
	void deleteBackingSpacesSpecifiedInTargetProperties() {
		...
		// configured backing services
		BackingServices servicesWithTarget = BackingServices.builder().backingServices(this.backingServices).build();
		...
		BackingApplications appsWithTarget = BackingApplications.builder()
			.backingApplications(this.backingApps)
			.build();
	...
	void deleteServiceInstanceSucceedsWhenBackingServicesDifferFromConfiguration() {
		...
		// configured backing services
		given(this.targetService.addToBackingServices(eq(this.backingServices2), eq(this.targetSpec),
				eq("service-instance-id")))
			.willReturn(Mono.just(this.backingServices2));
		...
		given(this.backingServicesProvisionService.deleteServiceInstance(argThat((backingServices) -> {
			...
			boolean spaceMatch0 = "my-space2"
				.equals(backingServices.get(0).getProperties().get(DeploymentProperties.TARGET_PROPERTY_KEY));
			boolean nameMatch1 = "different-service-instance".equals(backingServices.get(1).getServiceInstanceName());
			boolean spaceMatch1 = "TargetSpace"
				.equals(backingServices.get(1).getProperties().get(DeploymentProperties.TARGET_PROPERTY_KEY));
		...
		given(this.backingSpaceManagementService.deleteTargetSpaces(eq(Collections.singletonList("my-space2"))))
			.willReturn(Flux.just("space-name"));

		StepVerifier.create(this.deleteServiceInstanceWorkflow.delete(request, response))
			.expectNext()
			.expectNext()
			.verifyComplete();
	...
	private Map<String, String> getPropertiesWithSpace(String customSpace) {
		return Collections.singletonMap(DeploymentProperties.TARGET_PROPERTY_KEY, customSpace);
	}
