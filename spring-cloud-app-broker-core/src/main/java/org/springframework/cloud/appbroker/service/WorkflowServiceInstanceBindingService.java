...
public class WorkflowServiceInstanceBindingService implements ServiceInstanceBindingService {
	...
	public Mono<CreateServiceInstanceBindingResponse> createServiceInstanceBinding(
		...
		return invokeCreateResponseBuilders(request).flatMap((response) -> {
			if (response.isAsync()) {
				return Mono.just(response)
					.publishOn(Schedulers.parallel())
					.doOnNext((r) -> create(request, r).subscribe());
			}
			else {
	...
	private Mono<Void> create(CreateServiceInstanceBindingRequest request,
		...
		return this.stateRepository
			.saveState(request.getServiceInstanceId(), request.getBindingId(), OperationState.IN_PROGRESS,
					"create service instance binding started")
			.thenMany(invokeCreateWorkflows(request, response).doOnRequest((l) -> {
				...
				.doOnError((e) -> LOG
					.error(String.format("Error creating service instance binding. error=%s", e.getMessage()), e)))
			.thenEmpty(this.stateRepository
				.saveState(request.getServiceInstanceId(), request.getBindingId(), OperationState.SUCCEEDED,
						"create service instance binding completed")
				.then())
			.onErrorResume((exception) -> this.stateRepository
				.saveState(request.getServiceInstanceId(), request.getBindingId(), OperationState.FAILED,
	...
	private Flux<Void> invokeCreateWorkflows(CreateServiceInstanceBindingRequest request,
		...
		return Flux.defer(() -> {
			if (isAppBindingRequest(request)) {
				return Flux.fromIterable(this.createServiceInstanceAppBindingWorkflows)
					.filterWhen((workflow) -> workflow.accept(request))
					.concatMap(
							(workflow) -> workflow.create(request, (CreateServiceInstanceAppBindingResponse) response));
			}
			else if (isRouteBindingRequest(request)) {
				return Flux.fromIterable(this.createServiceInstanceRouteBindingWorkflows)
					.filterWhen((workflow) -> workflow.accept(request))
	...
	public Mono<DeleteServiceInstanceBindingResponse> deleteServiceInstanceBinding(
		...
		return invokeDeleteResponseBuilders(request).flatMap((response) -> {
			if (response.isAsync()) {
				return Mono.just(response)
					.publishOn(Schedulers.parallel())
					.doOnNext((r) -> delete(request, r).subscribe());
			}
			else {
	...
	private Mono<DeleteServiceInstanceBindingResponse> invokeDeleteResponseBuilders(
			DeleteServiceInstanceBindingRequest request) {
		AtomicReference<DeleteServiceInstanceBindingResponseBuilder> responseBuilder = new AtomicReference<>(
				DeleteServiceInstanceBindingResponse.builder());

		return Flux.fromIterable(this.deleteServiceInstanceBindingWorkflows)
			.filterWhen((workflow) -> workflow.accept(request))
			.flatMap(
	...
	private Mono<Void> delete(DeleteServiceInstanceBindingRequest request,
		...
		return this.stateRepository
			.saveState(request.getServiceInstanceId(), request.getBindingId(), OperationState.IN_PROGRESS,
					"delete service instance binding started")
			.thenMany(invokeDeleteWorkflows(request, response).doOnRequest((l) -> {
				...
				.doOnError((e) -> LOG
					.error(String.format("Error deleting service instance binding. error=%s", e.getMessage()), e)))
			.thenEmpty(this.stateRepository
				.saveState(request.getServiceInstanceId(), request.getBindingId(), OperationState.SUCCEEDED,
						"delete service instance binding completed")
				.then())
			.onErrorResume((exception) -> this.stateRepository
				.saveState(request.getServiceInstanceId(), request.getBindingId(), OperationState.FAILED,
						exception.getMessage())
	...
	public Mono<GetLastServiceBindingOperationResponse> getLastOperation(
		...
		return this.stateRepository.getState(request.getServiceInstanceId(), request.getBindingId())
			.doOnError(
					(exception) -> Mono.error(new ServiceInstanceBindingDoesNotExistException(request.getBindingId())))
