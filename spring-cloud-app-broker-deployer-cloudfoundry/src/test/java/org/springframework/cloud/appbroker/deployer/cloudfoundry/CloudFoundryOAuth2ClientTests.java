...
class CloudFoundryOAuth2ClientTests {
	...
	@BeforeEach
	void setUp() {
		given(this.uaaClient.clients()).willReturn(this.clients);

		this.oAuth2Client = new CloudFoundryOAuth2Client(this.uaaClient);
	}
	...
	void createClient() {
		CreateClientRequest clientsRequest = CreateClientRequest.builder()
			...
			.authorizedGrantTypes(GrantType.CLIENT_CREDENTIALS, GrantType.AUTHORIZATION_CODE, GrantType.PASSWORD,
					GrantType.IMPLICIT, GrantType.REFRESH_TOKEN)
		...
		given(this.clients.create(clientsRequest)).willReturn(Mono.just(clientsResponse));
		...
		StepVerifier.create(this.oAuth2Client.createClient(request))
			.assertNext((response) -> assertResponse(response.getClientId(), response.getClientName(),
					response.getScopes(), response.getAuthorities(), response.getGrantTypes()))
			.verifyComplete();
	}
	...
	void deleteClient() {
		...
		given(this.clients.delete(clientsRequest)).willReturn(Mono.just(clientsResponse));
		...
		StepVerifier.create(this.oAuth2Client.deleteClient(request))
			.assertNext((response) -> assertResponse(response.getClientId(), response.getClientName(),
					response.getScopes(), response.getAuthorities(), response.getGrantTypes()))
			.verifyComplete();
	}

	private void assertResponse(String clientId, String name, List<String> scopes, List<String> authorities,
		...
		assertThat(authorizedGrantTypes).contains("client_credentials", "authorization_code", "password", "implicit",
				"refresh_token");
	}
