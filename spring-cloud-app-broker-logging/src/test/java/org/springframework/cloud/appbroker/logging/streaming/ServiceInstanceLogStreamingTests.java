...
class ServiceInstanceLogStreamingTests {
	...
	void setUp() {
		...
		Envelope testEnvelope = Envelope.builder()
			...
			.log(Log.builder()
				...
				.build())
			.build();
		this.expectedEnvelope = new org.cloudfoundry.dropsonde.events.Envelope.Builder()
			...
			.build();
		this.actualEnvelope.set(testEnvelope);

		ReadResponse response = ReadResponse.builder()
			.envelopes(EnvelopeBatch.builder().batch(testEnvelope).build())
			.build();

		given(this.logCacheClient.read(any(ReadRequest.class))).willReturn(Mono.just(response));

		given(this.cloudFoundryClient.applicationsV2()
			...
			.willReturn(Mono.just(GetApplicationResponse.builder()
				.entity(ApplicationEntity.builder().name("test-app").build())
				.build()));
	}
	...
	void shouldPublishWebSocketEndpoint() {
		...
		await()
			.untilAsserted(() -> assertThat(Base64.getDecoder().decode(this.actualEnvelope.get().getLog().getPayload()))
				.isEqualTo(Base64.getDecoder().decode(this.expectedEnvelope.logMessage.message.toByteArray())));
	...
	void shouldPublishEventOnDisconnect() {
		...
		await().untilAsserted(() -> assertThat(this.actualEnvelope.get()).isNotNull());
	...
	void shouldStopStreamingIfNoClient() {
		// CLI plugin doesn't always handle disconnect gracefully, so sometimes it is
		// possible that log stream is
		// being published but there is no listener. In this case log streaming should be
		// stopped.
		...
		this.applicationEventPublisher
			.publishEvent(new ServiceInstanceLogEvent(this, SERVICE_INSTANCE_ID, this.expectedEnvelope));
		await().untilAsserted(() -> assertThat(LogStreamingTestApp.getReceivedStopEventServiceInstanceId())
			.isEqualTo(SERVICE_INSTANCE_ID));
	}

	private Disposable connectToLogsStreamEndpoint() {
		URI uri = URI.create("ws://localhost:" + this.port + "/logs/" + SERVICE_INSTANCE_ID + "/stream");
	...
	private WebSocketHandler getWebSocketHandler() {
		return (session) -> session.receive().doOnNext((message) -> {
			DataBuffer buffer = message.getPayload();
			this.actualEnvelope.set(Envelope.builder()
