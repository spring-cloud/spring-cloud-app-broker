...
class CloudFoundryAppDeployerDeleteSpaceTests {
	...
	void deleteSpaceIfExists() {
		given(this.clientOrganizations.listSpaces(ListOrganizationSpacesRequest.builder()
			...
			.page(1)
			.build()))
			.willReturn(Mono.just(ListOrganizationSpacesResponse.builder()
				.resource(SpaceResource.builder()
					.entity(SpaceEntity.builder().name("test-space-name").build())
					.metadata(Metadata.builder().id("test-space-id").build())
					.build())
				.build()));

		given(this.cloudFoundryClient.spaces()).willReturn(this.clientSpaces);
		given(this.clientSpaces.delete(DeleteSpaceRequest.builder().spaceId("test-space-id").recursive(true).build()))
			.willReturn(Mono.empty());

		DeleteBackingSpaceRequest request = DeleteBackingSpaceRequest.builder().name("test-space-name").build();

		StepVerifier.create(this.appDeployer.deleteBackingSpace(request))
	...
	void doNothingIfSpaceDoesNotExist() {
		given(this.clientOrganizations.listSpaces(ListOrganizationSpacesRequest.builder()
			...
			.organizationId("default-org-id")
			.page(1)
			.build())).willReturn(Mono.just(ListOrganizationSpacesResponse.builder().resources().build()));
