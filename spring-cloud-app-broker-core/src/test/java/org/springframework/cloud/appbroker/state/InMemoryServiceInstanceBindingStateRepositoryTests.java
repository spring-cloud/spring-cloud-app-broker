...
class InMemoryServiceInstanceBindingStateRepositoryTests {
	...
	void saveAndGet() {
		StepVerifier
			.create(this.stateRepository.saveState("foo-service", "foo-binding", OperationState.IN_PROGRESS, "bar"))
		...
		StepVerifier.create(this.stateRepository.getState("foo-service", "foo-binding"))
			.assertNext((serviceInstanceState) -> {
				assertThat(serviceInstanceState.getOperationState()).isEqualTo(OperationState.IN_PROGRESS);
				assertThat(serviceInstanceState.getDescription()).isEqualTo("bar");
				assertThat(serviceInstanceState.getLastUpdated()).isInSameMinuteWindowAs(new Date());
	...
	void saveAndRemove() {
		...
		StepVerifier.create(this.stateRepository.removeState("foo-service", "foo-binding"))
			.assertNext((serviceInstanceState) -> {
				assertThat(serviceInstanceState.getOperationState()).isEqualTo(OperationState.IN_PROGRESS);
				assertThat(serviceInstanceState.getDescription()).isEqualTo("bar");
				assertThat(serviceInstanceState.getLastUpdated()).isInSameMinuteWindowAs(new Date());
	...
	void updateState() {
		...
		StepVerifier
			.create(this.stateRepository.saveState("foo-service", "foo-binding", OperationState.SUCCEEDED, "bar"))
			.assertNext((serviceInstanceState) -> {
	...
	void getWithNullBindingKey() {
		StepVerifier.create(this.stateRepository.getState(null, null))
			.expectError(IllegalArgumentException.class)
			.verify();
	}
	...
	void ensureConcurrency() {
		StepVerifier
			.create(Flux.range(0, 100_000)
				...
				.flatMap((value) -> this.stateRepository
					.saveState("foo-service" + value, "foo-binding" + value, OperationState.IN_PROGRESS, "bar")
					.then(this.stateRepository.removeState("foo-service" + value, "foo-binding" + value))))
