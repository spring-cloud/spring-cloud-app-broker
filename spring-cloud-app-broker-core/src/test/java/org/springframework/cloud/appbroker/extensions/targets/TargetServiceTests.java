...
class TargetServiceTests {
	...
	void shouldAddProperties() {
		...
		List<BackingApplication> updatedBackingApplications = this.targetService
			.addToBackingApplications(Collections.singletonList(backingApplication), targetSpec, "service-id")
			.block();
	...
	void shouldAddShortNameAsServiceInstanceGuidSuffix() {
		...
		StepVerifier
			.create(this.targetService.addToBackingApplications(Collections.singletonList(backingApplication),
					targetSpec, serviceInstanceId))
