...
class AppDeploymentInstanceWorkflowTests {
	...
	void setUp() {
		this.backingApps = BackingApplications.builder()
			.backingApplication(BackingApplication.builder().name("app1").path("https://myfiles/app.jar").build())
			.build();

		BackingServices backingServices = BackingServices.builder()
			.backingService(BackingService.builder().name("service1").plan("plan1").build())
			.build();
		...
		this.workflow = new AppDeploymentInstanceWorkflow(brokeredServices);
	}

	@Test
	void acceptWithMatchingService() {
		ServiceDefinition serviceDefinition = buildServiceDefinition("service1", "plan1");
		StepVerifier.create(this.workflow.accept(serviceDefinition, serviceDefinition.getPlans().get(0)))
	...
	void getBackingAppForServiceSucceeds() {
		ServiceDefinition serviceDefinition = buildServiceDefinition("service1", "plan1");
		StepVerifier
			.create(this.workflow.getBackingApplicationsForService(serviceDefinition,
					serviceDefinition.getPlans().get(0)))
			.assertNext((actual) -> assertThat(actual).isEqualTo(this.backingApps).isNotSameAs(this.backingApps))
			.verifyComplete();
	}
