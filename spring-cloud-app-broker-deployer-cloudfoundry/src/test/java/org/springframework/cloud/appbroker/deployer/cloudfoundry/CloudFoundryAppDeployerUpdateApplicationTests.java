...
class CloudFoundryAppDeployerUpdateApplicationTests {
	...
	void setUp() {
		...
		given(this.cloudFoundryOperations.domains()).willReturn(this.domains);
		given(this.cloudFoundryOperations.spaces()).willReturn(this.spaces);
		...
		given(this.cloudFoundryClient.builds()).willReturn(this.builds);
		...
		given(this.operationsApplications.get(any())).willReturn(Mono.just(createApplicationDetail()));

		given(this.applicationsV2.summary(any(SummaryApplicationRequest.class)))
		...
		given(this.packages.create(any())).willReturn(Mono.just(CreatePackageResponse.builder()
		...
		given(this.packages.upload(any())).willReturn(Mono.just(UploadPackageResponse.builder()
			.data(BitsData.builder().build())
			.state(PackageState.READY)
			.type(PackageType.BITS)
		...
		given(this.packages.get(any())).willReturn(Mono.just(GetPackageResponse.builder()
			.data(BitsData.builder().build())
			.state(PackageState.READY)
			.type(PackageType.BITS)
			.createdAt("DATETIME")
		...
		given(this.applicationsV3.listPackages(any())).willReturn(Mono.just(ListApplicationPackagesResponse.builder()
			...
			.build()));
		given(this.applicationsV3.update(any()))
		...
		given(this.builds.create(any())).willReturn(Mono.just(CreateBuildResponse.builder()
			...
			.id("build-id")
			.build()));
		given(this.builds.get(any())).willReturn(Mono.just(GetBuildResponse.builder()
		...
		given(this.deploymentsV3.create(any())).willReturn(Mono.just(CreateDeploymentResponse.builder()
			.state(DeploymentState.DEPLOYED)
			.createdAt("DATETIME")
			.id("deployment-id")
			.build()));
		given(this.deploymentsV3.get(any())).willReturn(Mono.just(GetDeploymentResponse.builder()
			.state(DeploymentState.DEPLOYED)
	...
	@Test
	void updateApp() {
		given(this.applicationsV2.update(any())).willReturn(Mono.just(UpdateApplicationResponse.builder().build()));

		UpdateApplicationRequest request = UpdateApplicationRequest.builder().name(APP_NAME).path(APP_PATH).build();

		StepVerifier.create(this.appDeployer.update(request))
	...
	void updateAppWithTarget() {
		...
		Map<String, String> properties = Collections.singletonMap(DeploymentProperties.TARGET_PROPERTY_KEY,
				"service-instance-id");
		...
		then(this.operationsUtils).should().getOperations(properties);
		then(this.operationsUtils).shouldHaveNoMoreInteractions();
	}

	@Test
	void updateAppProperties() {
		ArgumentCaptor<org.cloudfoundry.client.v2.applications.UpdateApplicationRequest> updateApplicationRequestCaptor = ArgumentCaptor
			.forClass(org.cloudfoundry.client.v2.applications.UpdateApplicationRequest.class);

		given(this.applicationsV2.update(updateApplicationRequestCaptor.capture()))
	...
	void updateAppWithUpgrade() {
		...
		then(this.applicationsV2).should().summary(any(SummaryApplicationRequest.class));
		then(this.applicationsV2).should().update(updateApplicationRequestCaptor.capture());
		assertThat(updateApplicationRequestCaptor.getValue().getEnvironmentJsons().get("SPRING_APPLICATION_JSON"))
	...
	void updateAppWithNoNewServices() {
		...
		then(this.applicationsV2).should().summary(SummaryApplicationRequest.builder().applicationId(APP_ID).build());
		then(this.applicationsV2).should().update(argThat((arg) -> APP_ID.equals(arg.getApplicationId())));
		then(this.operationsServices).shouldHaveNoMoreInteractions();
		then(this.applicationsV2).shouldHaveNoMoreInteractions();
	}

	@Test
	void updateAppWithNewServices() {
		given(this.operationsServices.bind(any(BindServiceInstanceRequest.class))).willReturn(Mono.empty());
		given(this.applicationsV2.update(any())).willReturn(Mono.just(UpdateApplicationResponse.builder().build()));
		...
		then(this.applicationsV2).should().summary(SummaryApplicationRequest.builder().applicationId(APP_ID).build());
		then(this.operationsServices).should()
	...
	void updateAppWithHostAndDomain() {
		...
		then(this.routes).should()
			.create(CreateRouteRequest.builder().domainId("myDomainComId").spaceId("space-id").host("my.host").build());
		then(this.applicationsV2).should()
	...
	void updateAppWithHostDomainAndDomains() {
		mockDomainsAndRoutesUpdate();
		given(this.domains.list()).willReturn(Flux.fromStream(IntStream.range(1, 4)
			.mapToObj(
					(i) -> Domain.builder().name("domain" + i).id("domain" + i + "-id").status(Status.OWNED).build())));
		...
		ArgumentCaptor<CreateRouteRequest> createRouteRequestArgumentCaptor = ArgumentCaptor
			.forClass(CreateRouteRequest.class);
		then(this.routes).should(times(3)).create(createRouteRequestArgumentCaptor.capture());
	...
	void updateAppWithRoutes() {
		...
		Map<String, String> properties = Collections.singletonMap("routes", "foo.domain1,bar.domain2");
		...
		ArgumentCaptor<CreateRouteRequest> createRouteRequestArgumentCaptor = ArgumentCaptor
			.forClass(CreateRouteRequest.class);
		then(this.routes).should(times(2)).create(createRouteRequestArgumentCaptor.capture());
	...
	void updateAppWithDomainAndNoHost() {
		...
		given(this.domains.list()).willReturn(getDomains());

		given(this.spaces.get(any())).willReturn(
				Mono.just(SpaceDetail.builder().id("space-id").name("space-name").organization("org-name").build()));
		given(this.routes.create(any())).willReturn(
				Mono.just(CreateRouteResponse.builder().metadata(Metadata.builder().id("route-id").build()).build()));
		given(this.applicationsV2.associateRoute(any())).willReturn(Mono.empty());
		...
		then(this.routes).should()
			.create(CreateRouteRequest.builder().domainId("myDomainComId").spaceId("space-id").host(null).build());
		then(this.applicationsV2).should()
			.associateRoute(
					AssociateApplicationRouteRequest.builder().applicationId(APP_ID).routeId("route-id").build());
	}
	...
	void updateAppWithNotExistingDomain() {
		...
		StepVerifier.create(this.appDeployer.update(request)).expectError(RuntimeException.class).verify();

		then(this.routes).shouldHaveNoInteractions();
	}
	...
	private Flux<Domain> getDomains() {
		return Flux.just(
				Domain.builder()
					...
					.type("internal")
					.build(),
				Domain.builder().id("myDomainDefaultId").name("my.domain.default.com").status(Status.SHARED).build(),
				Domain.builder().id("myDomainComId").name("my.domain.com").status(Status.OWNED).build());
	}
	...
	private void verifyRouteCreatedAndMapped(String domainId, String host, String spaceId, String appId) {
		then(this.routes).should()
			.create(CreateRouteRequest.builder().domainId(domainId).spaceId(spaceId).host(host).build());
	...
	private void mockDomainsAndRoutesUpdate() {
		...
		given(this.spaces.get(any())).willReturn(
				Mono.just(SpaceDetail.builder().id("space-id").name("space-name").organization("org-name").build()));
		given(this.domains.list()).willReturn(getDomains());
		given(this.routes.create(any())).willAnswer((invocation) -> {
			CreateRouteRequest createRouteRequest = invocation.getArgument(0);
			return Mono.just(CreateRouteResponse.builder()
				.metadata(Metadata.builder().id(getRouteIdForDomain(createRouteRequest.getDomainId())).build())
				.build());
		});
		given(this.applicationsV2.associateRoute(any())).willReturn(Mono.empty());
	}
