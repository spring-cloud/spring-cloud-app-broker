...
import static org.springframework.cloud.appbroker.integration.CreateInstanceWithCustomTargetComponentTests.APP_NAME;
import static org.springframework.cloud.appbroker.integration.CreateInstanceWithCustomTargetComponentTests.BACKING_SERVICE_NAME;
import static org.springframework.cloud.appbroker.integration.CreateInstanceWithCustomTargetComponentTests.BACKING_SI_NAME;
...
@ContextConfiguration(classes = CreateInstanceWithCustomTargetComponentTests.CustomConfig.class)
class CreateInstanceWithCustomTargetComponentTests extends WiremockComponentTests {
	...
	void pushAppWithServicesInSpace() {
		...
		this.cloudControllerFixture.stubFindSpaceV3(customSpace, customSpaceGuid);
		this.cloudControllerFixture.stubCreateSpace(customSpace, customSpaceGuid);
		this.cloudControllerFixture.stubAssociatePermissions();
		this.cloudControllerFixture.stubPushApp(APP_NAME);

		// given services are available in the marketplace
		this.cloudControllerFixture.stubServiceExistsInSpace(BACKING_SERVICE_NAME, BACKING_PLAN_NAME, customSpaceGuid);

		// will create and bind the service instance
		this.cloudControllerFixture.stubCreateServiceInstance(BACKING_SI_NAME);
		this.cloudControllerFixture.stubCreateServiceBinding(APP_NAME, BACKING_SI_NAME);
		this.cloudControllerFixture.stubServiceInstanceExistsInSpace(BACKING_SI_NAME, customSpaceGuid);

		// when a service instance is created
		given(this.brokerFixture.serviceInstanceRequest()).when()
			.put(this.brokerFixture.createServiceInstanceUrl(), serviceInstanceId)
			.then()
	...
	protected static class CustomConfig {
		...
		static final class CustomSpaceTarget extends TargetFactory<CustomSpaceTarget.Config> {
			...
			CustomSpaceTarget(CustomSpaceService customSpaceService) {
				super(Config.class);
			...
			private ArtifactDetails apply(Map<String, String> properties, String name, String serviceInstanceId) {
				String space = this.customSpaceService.retrieveSpaceName();
