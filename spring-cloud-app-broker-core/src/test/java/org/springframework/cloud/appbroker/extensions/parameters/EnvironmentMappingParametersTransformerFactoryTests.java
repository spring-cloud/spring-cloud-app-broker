...
class EnvironmentMappingParametersTransformerFactoryTests {

	private ParametersTransformer<BackingApplication> transformer;

	@BeforeEach
	void setUp() {
		this.transformer = new EnvironmentMappingParametersTransformerFactory()
			.createWithConfig((config) -> config.setInclude("parameter1,parameter2,parameter4,parameter5"));
	}
	...
	void parametersAreMappedToApplicationEnvironment() {
		...
		StepVerifier.create(this.transformer.transform(backingApplication, parameters))
