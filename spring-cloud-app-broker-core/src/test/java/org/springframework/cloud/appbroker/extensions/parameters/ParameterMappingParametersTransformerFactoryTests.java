...
class ParameterMappingParametersTransformerFactoryTests {

	private ParametersTransformer<BackingService> transformer;

	@BeforeEach
	void setUp() {
		this.transformer = new ParameterMappingParametersTransformerFactory()
			.createWithConfig((config) -> config.setInclude("parameter1,parameter2"));
	}
	...
	void parametersOverrideApplicationEnvironment() {
		...
		BackingService backingService = BackingService.builder().parameters(expectedParameters).build();

		StepVerifier.create(this.transformer.transform(backingService, inputParameters))
